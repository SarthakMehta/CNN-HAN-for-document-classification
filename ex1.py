...
def encode(X,Y):
    global tokenizer,max_len_word,max_len_char
    train_x= np.zeros((len(X),max_len_word,max_len_char,len(tokenizer.word_index)+1))
    train_y=np.zeros((len(X),max(Y)))
    for doc in range(0,len(X)):
        train_y[doc][Y[doc]-1]=1
        wl = tokenizer.texts_to_sequences(text_to_word_sequence(X[doc]))
        for word in range(0,len(wl)):
            for cha in range(0,len(wl[word])):
                train_x[doc][word][cha][wl[word][cha]] =1
...
#
permutation = random.sample(range(0,len(Train_x)),len(Train_x))
#
...
train_x,train_y = encode(Train_x,Train_y)

out=''
csv_logger = CSVLogger('log2.csv', append=True, separator=';')
    
inp_shape= len(tokenizer.word_index)+1
out_shape = 4
...
#model.fit_generator(encode(Train_x[0:],Train_y[0:20000]), steps_per_epoch=25000, epochs=10,callbacks=[csv_logger],validation_data=encode(Train_x[20000:22500],Train_y[20000:22500]),validation_steps=2500)
